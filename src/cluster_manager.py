...
class ClusterManager:
    ...
    def submit_job(self, model: str, dataset: str, gpus: int,
        ...
        if gpus > self.available_gpus:
            raise ValueError(f"Requested {gpus} GPUs, only {self.available_gpus} available")
        ...
        print(f"📊 Cluster: {len(self.active_jobs)} active jobs, {self.available_gpus:,} GPUs available")
